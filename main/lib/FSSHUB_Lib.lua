-- [[ FSSHUB LIBRARY SOURCE V4.1 (BUG FIXES) ]] --
-- Update: Fixed UI Disappearing Bug, Safe Object Creation, Stable Animations
...
-- [INTERNAL] SAFE CREATE FUNCTION
-- Memastikan Parent diset TERAKHIR untuk mencegah glitch rendering
local function Create(class, props)
    local inst = Instance.new(class)
    local parent = props.Parent
    props.Parent = nil -- Hapus parent dari list properti sementara
    
    for i, v in pairs(props) do 
        inst[i] = v 
    end
    
    if parent then inst.Parent = parent end -- Set parent terakhir
    return inst
end
...
function FSSHUB:Notify(text, type, duration)
    ...
    Create("TextLabel", {Parent = Notif, Text = type == "error" and "!" or "✓", TextColor3 = Color, Font = Enum.Font.GothamBold, TextSize = 24, Size = UDim2.new(0, 40, 1, 0), BackgroundTransparency = 1})
...
function FSSHUB:AuthWindow(options)
    ...
    -- Main Card (Fixed Animation Logic)
    -- Kita mulai dengan ukuran FULL, tapi Scale 0. Ini mencegah layout glitch.
    local Main = Create("Frame", {
        Parent = Screen, 
        BackgroundColor3 = FSSHUB.Theme.Background, 
        Size = UDim2.new(0, 380, 0, 220), -- Ukuran tetap
        Position = UDim2.new(0.5, 0, 0.5, 0), 
        AnchorPoint = Vector2.new(0.5, 0.5),
        ZIndex = 1
        -- ClipsDescendants dihapus agar aman
    })
    Create("UICorner", {Parent = Main, CornerRadius = UDim.new(0, 10)})
    AddStroke(Main, FSSHUB.Theme.Accent, 2)
    
    -- Gunakan UIScale untuk animasi Pop-Up (Lebih stabil daripada Resize)
    local Scale = Create("UIScale", {Parent = Main, Scale = 0})
    TweenService:Create(Scale, TweenInfo.new(0.5, Enum.EasingStyle.Elastic), {Scale = 1}):Play()
    
    MakeDraggable(Main, Main)

    -- Elements (ZIndex Explicit)
    local Title = Create("TextLabel", {
        Parent = Main, Text = options.Title or "AUTHENTICATION", TextColor3 = FSSHUB.Theme.Accent, 
        Font = Enum.Font.GothamBold, TextSize = 20, Size = UDim2.new(1, 0, 0, 40), 
        Position = UDim2.new(0, 0, 0, 10), BackgroundTransparency = 1, ZIndex = 2
    })
    
    local Status = Create("TextLabel", {
        Parent = Main, Text = options.Status or "Please enter your key below", TextColor3 = FSSHUB.Theme.TextDim, 
        Font = Enum.Font.Gotham, TextSize = 12, Size = UDim2.new(1, 0, 0, 20), 
        Position = UDim2.new(0, 0, 0, 35), BackgroundTransparency = 1, ZIndex = 2
    })

    local KeyInput = Create("TextBox", {
        Parent = Main, BackgroundColor3 = FSSHUB.Theme.Card, TextColor3 = FSSHUB.Theme.Accent,
        Font = Enum.Font.Mono, TextSize = 14, Size = UDim2.new(0.8, 0, 0, 40), 
        Position = UDim2.new(0.1, 0, 0.35, 0), Text = "", PlaceholderText = "Paste Key Here...",
        ZIndex = 2
    })
    ...
    local BtnContainer = Create("Frame", {
        Parent = Main, BackgroundTransparency = 1, Size = UDim2.new(0.8, 0, 0, 40), 
        Position = UDim2.new(0.1, 0, 0.65, 0), ZIndex = 2
    })
    ...
    local function CreateAuthBtn(text, color, callback)
        local Btn = Create("TextButton", {
            ...
            BackgroundColor3 = color, Size = UDim2.new(0.5, -5, 1, 0), AutoButtonColor = false, ZIndex = 3
        })
    ...
    local Close = Create("TextButton", {
        Parent = Main, Text = "×", TextColor3 = FSSHUB.Theme.Error, BackgroundTransparency = 1, 
        Font = Enum.Font.GothamBold, TextSize = 24, Size = UDim2.new(0, 30, 0, 30), 
        Position = UDim2.new(1, -30, 0, 0), ZIndex = 2
    })
    Close.MouseButton1Click:Connect(function() Screen:Destroy() end)
end

-- [MODE 2] MAIN WINDOW (Standard)
function FSSHUB:Window(title)
    ...
    Create("Frame", {Parent = Sidebar, BackgroundColor3 = FSSHUB.Theme.Card, Size = UDim2.new(0, 10, 1, 0), Position = UDim2.new(1, -10, 0, 0), BorderSizePixel=0})

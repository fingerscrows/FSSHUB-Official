-- [[ FSSHUB: UI MANAGER V2.1 (SYNCED COUNTDOWN) ]] --

local UIManager = {}
local LIB_URL = "https://raw.githubusercontent.com/fingerscrows/fsshub-official/main/main/lib/FSSHUB_Lib.lua"
local success, Library = pcall(function() return loadstring(game:HttpGet(LIB_URL))() end)

if not success or not Library then warn("FSSHUB: Library failed!") return UIManager end
Library:Init()
...
function UIManager.Build(GameConfig, AuthData)
    ...
    if AuthData then
        ProfileTab:Paragraph("User Information", "License: " .. AuthData.Type .. "\nGame: " .. GameConfig.Name)
        
        local TimerLabel = ProfileTab:Label("Syncing Time...")
        
        -- Logic Countdown Stabil
        task.spawn(function()
            -- Jika Expiry 0 atau sangat besar, anggap Unlimited
            if not AuthData.Expiry or AuthData.Expiry == 0 or AuthData.Expiry > 4000000000 then
                TimerLabel.Text = "Status: LIFETIME / UNLIMITED"
                return
            end

            while true do
                -- Gunakan os.time() (UTC) untuk sinkronisasi global
                ...
                if timeLeft > 0 then
                    ...
                    TimerLabel.Text = "LICENSE EXPIRED (Re-Login)"
                end
                task.wait(1)
            end
        end)
    else
        ProfileTab:Paragraph("Status", "Dev Mode")
    end
    
    ProfileTab:Paragraph("Quick Guide", "• Right Ctrl: Hide UI\n• Settings: Unload Script")
    ...
    SettingsTab:Button("Unload Script", function()
        ...
        game.StarterGui:SetCore("SendNotification", {Title = "System", Text = "Script Unloaded"})
    ...
    game.StarterGui:SetCore("SendNotification", {Title = "FSSHUB", Text = GameConfig.Name .. " Loaded!"})
end
return UIManager
